...
void rightViewBT(Node* root,vector<int>&result,vector<int>&levelvisited,int level){
    if(root==NULL)  return;
    if(levelvisited[level]==0){
        result.push_back(root->data);
        levelvisited[level]=1;
    }
    rightViewBT(root->right,result,levelvisited,level+1);
    rightViewBT(root->left,result,levelvisited,level+1);
...
int main(){
    ...
    rightViewBT(root,result,levelvisited,0);
    cout<<"RightView Of BT is: ";
